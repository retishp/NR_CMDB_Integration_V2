...
# Check to see if CIs already exist in Service Now
if (os.path.exists('./snow_infra_cis.csv')):
    df_exist = pandas.read_csv('./snow_infra_cis.csv')
else:
    print("Missing config file : snow_infra_cis.csv")
    exit()

# Set the request parameters
url = snow_url + '/api/now/table/u_cmdb_ci_new_relic_infras2'
...
for index, row in df_infra.iterrows():
    ...
    df_check = df_exist[df_exist['u_guid'].str.contains(infra_dict["u_guid"])]
    if not df_check.empty:
        sys_id = df_check['sys_id'].iloc[0]
        if log_level == "Debug":
            print(sys_id)
        update_url = url + '/' + sys_id
        # count += 1
      
        # Do the HTTP request
        response = requests.put(update_url, auth=(
...
# Check to see if CIs already exist in Service Now
if (os.path.exists('./snow_apm_cis.csv')):
    df_exist = pandas.read_csv('./snow_apm_cis.csv')
else:
    print("Missing config file : snow_apm_cis.csv")
    exit()

# Set the request parameters
url = snow_url + '/api/now/table/u_cmdb_ci_nr_apm2'
...
for index, row in df_apm.iterrows():
    ...
    df_check = df_exist[df_exist['u_guid'].str.contains(apm_dict["u_guid"])]
    if log_level == "Debug":
        print("apm sys ids are: ")
    if not df_check.empty:
        sys_id = df_check['sys_id'].iloc[0]
        if log_level == "Debug":
            print(sys_id)
...
# Set the request parameters
url = snow_url + '/api/now/table/u_cmdb_ci_new_relic_infras2?sysparm_fields=sys_id%2C%20u_guid&sysparm_limit=500'
...
df_SNOW.to_csv("snow_infra_cis.csv", index=False)
...
# Set the request parameters
url = snow_url + '/api/now/table/u_cmdb_ci_nr_apm2?sysparm_fields=sys_id%2C%20u_guid&sysparm_limit=60'
...
df_apm.fillna('', inplace=True)
df_apm.to_csv("snow_apm_cis.csv", index=False)

# Setting up CI relationships

rel_url = snow_url + "/api/now/cmdb/instance/u_cmdb_ci_new_relic_infras2/"
        ...
        # Check for HTTP codes other than 200
            if response.status_code > 201:
                print('Status:', response.status_code, 'Headers:',
                      response.headers, 'Error Response:', response.json())
                exit()
print("Successfully inserted/updated New Relic data into Service Now")